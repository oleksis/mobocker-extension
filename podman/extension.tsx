// https://github.com/containers/podman-desktop-extension-minikube/blob/main/src/extension.ts
...
const client = podmanDesktopAPI.process;
...
export function App() {
  ...
  const [mobockerC, setMobockerC] = React.useState<string>(
    "podman-desktop-ext-mobocker-mobocker-extension-1"
  );
  const ddClient = useDockerDesktopClient();

  const checkBackendServiceStatus = async () => {
    const result = await ddClient.exec("podman", [
      "container",
      "inspect",
      "--format",
      "{{.State.Status}}",
